...
public class MethodOverloading {
	...
	public static void main(String[] args) {
		//메소드 오버로딩 : 하나의 메소드명에 매개 변수타입, 개수에 따라서 각각 다른 메소드가 호출됨 (Java)
		//메소드 오버라이딩 : 상속에서 부모 클래스의 메소드를 재정의 해서 사용
		
		//static : 객체 생성 없이 바로 호출해서 사용가능하도록 할수있도록 키워드
		...
		print1();		//static가 들어가 있고 동일한 클래스에 생성된 메소드는 바로 호출이 가능
	...
	//메소드 오버라이딩 : 메소드 이름은 동일, 매개변수타입, 매개변수 개수에 따라 해당 메소드를 호출
		// JVM이 동일한 메소드 이름의 데이터 타입이나 개수에 따라서 해당 메소드를 작동
	
		//주의 : 동일한 매개변수타입과 개수가 같으면 오류를 발생 시킴 
	...
	/*  오류발생 : 동일한 데이터 타입, 동일한 개수의 메소드가 존재하는 경우
	...
	*/
	...
	public static void print1 (int c, double d) {	//가능
