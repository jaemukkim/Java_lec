...
class Aaa {
	...
	static void print2() {	//메소드 (Static 메소드) : 객체 생성없이 호출 가능, 객체 생성후에도 호출 가능
		//static 메소드에서는 인스턴스 필드가 올수 없다. static 필드만 와야한다.
			//static 메소드는 객체 생성없이 호출이 되므로 인스턴스 필드가 올수 없다.
