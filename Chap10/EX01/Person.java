...
class Student_sub extends Student {
	...
	//Student_sub는 Person의 필드와 메소드와 Student의 필드와 메소드를 모두! 상속 받는다.
...
class Student extends Person {	//대학생 클래스, Person : 부모, Student 자식
	...
	int studentID;
	void goToSchool() {
		System.out.println("자식 클래스의 Student 메소드 goToSchool");
		}
	}
...
public class Person {	//상속 : 부모 클래스(필드, 메소드, 내부클래스)를 자식클래스에게 상속
	...
	public static void main(String[] args) {
		...
		Person ps = new Student(); // 업캐스팅, Student는 Person이다. (자식->부모)
//		Student sp = new Person(); // 오류, Person은 Student 이다(X) (부모->자식 X )
		Person pw = new Worker(); // 업캐스팅, Worker는 Person 이다.(O)
//		Worker wp = new Person(); // 오류, Person은 Worker이다 (X) 
		
		
		System.out.println("======Person <--Student <--Student_sub========");
		
		Student_sub sub1 = new Student_sub();	//가능
		//클래스 타입(Type)  객체명	 < 자식 클래스 >
		...
		//Student_sub는 Person 타입으로 업캐스팅, Person 클래스의 멤버만 사용 가능 (Student는 X)
		...
		//Person 과 Student의 필드와 메소드를 사용가능
		//Person 의 필드와 메소드
		ss1.name = "SES";
		ss1.age = 30;
		ss1.eat();
		...
		Student_sub ss2 = new Student_sub(); //Person, Student, Student_sub의 필드와 메소드 모두 사용가능.
		//Person 클래스의 필드와 메소드
		ss2.name = "봄여름가을겨울";
		ss2.age = 40;
		ss2.eat();
		...
		//상속 관계에서 객체 생성시 부모 데이터 타입으로 업캐스팅 가능.
			...
			Person ps4 = new Student_sub();	//Person 만 사용 가능
			...
			Student ss4 = new Student_sub(); //Person과 Student 사용 가능
			...
			Student_sub ss5 = new Student_sub();
				...
				//Student_sub는 Person과 Student와 Student_sub의 모든 필드와 메소드를 포함하고 있다
				//그중에서 Person, Student, Student_sub 클래스의 필드와 메소드 모두 사용가능
			...
			Student ps5 = (Student) ps4;  //ps4(Person, Student, Student_sub)의 데이터 타입은 Person
					//ps5는 Person, Student의 필드와 메소드 접근가능
			ps5.name = "다운캐스팅";
			ps5.age = 30;
			...
			ps6.name = "이용식";		// Person의 필드
			ps6.studentID = 20220310; //Student
			ps6.sub1 = "자식의 자식 클래스"; //Student_sub
