...
//		hTable1.put(3, "가가가");	//키는 중복해서 저장될 수 없다. 값을 넣을 경우 기존의 값이 수정이 된다.
		...
		hTable2.replace(4, "라라라");	//key 4는 존재하지 않는다. 작동X, 예외발생도X
		System.out.println(hTable2);
		
		//4. replace(K key, V oldValue, V newValue) : oldValue를 newValue로 수정
		hTable2.replace(1, "가가가", "나나나");
		hTable2.replace(2, "다다다", "라라라");	//동작안됨.. 예외발생도X ===> Key와 Value가 정확히 일치해야 수정됨. 
		System.out.println(hTable2);
		
		//5. get(Object Key) : Map의 값을 가져올때.		Key를 넣어줘야 Key에 할당된 Value를 출력.
		System.out.println(hTable2.get(1)); 	//주의 1은 index가 아니라 1번 Key의 값.
		System.out.println(hTable2.get(2));	//2번 key의 값
		System.out.println(hTable2.get(3));
		
		//6. containsKey(Object key) : 맵에 해당키가 존재하는지 확인, true, false
		System.out.println(hTable2.containsKey(1)); // 1번 key가 있느냐 / true
		System.out.println(hTable2.containsKey(5)); // 5번 key가 있느냐 / false
		...
		Set<Integer> keySet = hTable2.keySet();	//hTable2의 저장된 keySet을 추출 / 저장된 key만 
		...
		hTable2.remove(1);	//1번 key 삭제
		hTable2.remove(4);	//4번 없음 / 동작X
		System.out.println(hTable2);
		
		//12. remove (Object key, Object Value) : Key와 value가 정확하게 일치할때 삭제.
		hTable2.remove(2, "나다라");	
		hTable2.remove(3, "다다다");  //동작X / key와 value가 정확하게 일치해야함.
		System.out.println(hTable2);
