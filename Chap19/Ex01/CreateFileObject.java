...
//주의 : 이클립스에서 파일을 생성시 C:\ 에 생성할때 권한 문제로 인한 오류가 발생될 수 있다.
	// 1. 이클립스를 <<관리자권한>> 으로 실행
	// 2. 폴더 내부에 파일을 생성.
...
public class CreateFileObject {
	...
	public static void main(String[] args) throws IOException {	
		...
		//1. File 객체를 사용해서 폴더 생성.	해당 파일이나 폴더ㅏ 존재하지 않는 경우
		...
		if (!newFile.exists()) {		
			newFile.createNewFile(); 	//파일을 생성할 때 예외가 발생될 수 있다. 예외처리가 필요. 권한문제
		}
		System.out.println(newFile.exists());
		System.out.println();
		
		//3. 파일 구분자
		File newFile2 = new File("C:\\temp001\\newfile01.txt");	//Windows 10 하위 버젼    \\
		...
		//4. 절대 경로로 처리 : 다른 시스템으로 이동된 경우 그 시스템 환경에 맞도록 세팅이 필요.
