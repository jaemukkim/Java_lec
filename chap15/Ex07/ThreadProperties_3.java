...
//데몬 쓰레드 : setDaemon(true), 쓰레드를 start 할때 설정, 수정 변경은 불가.
	...
	// - 종료시점이 마지막 일반 쓰레드 종료될때 종료 외에는 일반쓰레드와 동일
	// - 가비지 컬렉트 (메모리의 사용되지 않는 객체를 가비지 컬렉트가 자동으로 객체를 제거)
	// 		프로세서가 종료시점에 자동저장, 화면 자동 갱신...
	// - 무한 루프를 사용해서 항시 대기하고 있다가 특정 조건이 만족하면 작업을 수행하고 다시 대기상태에 들어간다.		
...
//		thread1.start();
		...
		//2. Daemon 쓰레드 : 마지막 쓰레드가 종료되는 시점에 종료
