...
//쓰레드 상태
	...
	//4. TIME WAITING	: 특정 시간동안만 대기상태, Thread.sleep(1000); ,
				// a.join(1000)  <=== 현재 자신의 쓰레드는 1초 대기상태고, a 객체를 실행 상태로 먼저보냄.(양보)
				// 특정 시간이 종료되면 실행 상태로 감, 인터럽트가 발생되면 예외를 발생시키고 실행 상태로감.
	//5. BLOCK		: 동기화 처리에서 Lock 이 걸려서 접근이 잠시 차단된 상태, 키를 반납한 경우 키를 획득해서 실행 상태로 감.
	//6. WAITING		: 무한정 대기상태. 누가 깨워줘야 실행으로 갈수있따. 다른 쓰레드가 깨워줘야 한다.
