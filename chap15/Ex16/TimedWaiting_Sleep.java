...
//인스턴스명.join(1000) ===> TimedWaiting 상태로 이동 ===> 특정 시간(1초)이 경과되면 Runnable 상태로 이동
...
class MyThread extends Thread {
	...
	public void run() {
		try {
			Thread.sleep(3000);		// 3초동안 TimedWaithing 상태로 진입, 3초후에는 Runnable 상태로 이동
									// interrupt 가 발생되면 예외를 발생 시키고 Runnable 상태로 이동
