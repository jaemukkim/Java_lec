...
//제네릭 클래스의 상속 : 자식 클래스는 부모 클래스의 제너릭 변수(K,V,T,E)와 같거나 더 많아야 된다.
...
class Child1 <T> extends Parent<T> {	//제너릭 클래스를 상속받는 자식 클래스는 부모의 타입의 인자개수와 같거나 커야한다.
...
//		Parent<String> p = new Parent();	//뒤의 생성자 호출할 때는 생략 가능
		...
		//2. 자식 클래스 1
		Child1 <String> c1 = new Child1(); //<>생략됨
		...
		System.out.println(c2.getV());	//상속의 메소드
